...
def main():
    ...
    classifier = PyTorchClassifier(model=model, clip_values=(0, 1), loss=criterion,
                                   optimizer=opt, input_shape=[128, 3, 32, 32], nb_classes=10)

    attack = ProjectedGradientDescent(classifier=classifier,
                                      eps=cnfg['pgd']['epsilon']/255.,
                                      eps_step=cnfg['pgd']['alpha']/255.,
                                      max_iter=cnfg['pgd']['iter'],
                                      num_random_init=cnfg['pgd']['restarts'],
                                      batch_size=cnfg['data']['batch_size']
                                      )
